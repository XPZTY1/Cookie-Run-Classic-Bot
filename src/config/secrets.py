...
if getattr(sys, "frozen", False):
    ...
    # ไฟล์ secrets.py อยู่ที่ src/config/secrets.py -> ถอยขึ้น 2 ชั้นไป Project Root
    # (เดิมใช้ dirname เดียว ทำให้ BASE_DIR ชี้ไป src/config/ -> โหลด .env ผิดที่)
...
def save_secret(key, value):
    ...
    try:
        ...
        # propagate ค่าที่เปลี่ยนไปยัง settings.config ด้วย
        # เพราะ settings.py import ค่าตอนโหลดแล้วเก็บเป็น module-level var
        try:
            import src.config.settings as _cfg
            if env_key == "ADB_DEVICE_ID" and value:
                _cfg.DEVICE_ID = value
            elif env_key == "ADB_PATH":
                _cfg.ADB_PATH = _cfg.find_adb_path()
        except Exception:
            pass
        return True
    except Exception as e:
