...
from src.config.settings import ADB_PATH, DEVICE_ID
...
def adb_connect(target_device=None):
    ...
    for line in output.splitlines():
        ...
        if len(parts) >= 2 and parts[1] == "device":
            dev = parts[0]
            # exact match ก่อน (เช่น 127.0.0.1:5559)
            if dev == device_id:
                matched_device = dev
                break
            # fallback: เปรียบเทียบเฉพาะหมายเลขพอร์ต เพื่อรองรับ emulator-5554 vs 127.0.0.1:5554
            # โดยไม่ใช้ substring เพราะอาจ match ผิดตัว (เช่น 5555 ตรงกับ 55550)
            try:
                dev_port = dev.split(":")[-1]
                target_port = device_id.split(":")[-1]
                if dev_port == target_port and len(dev_port) == len(target_port):
                    matched_device = dev
                    break
