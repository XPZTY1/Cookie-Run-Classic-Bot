        ...
        if os.path.exists(stats_path):
            try:
                with open(stats_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                # ตรวจสอบว่ามี key ที่จำเป็นครบถ้วน (ป้องกัน KeyError ถ้าไฟล์เก่า/เสีย)
                required_keys = ["all_time_runs", "all_time_success", "all_time_watchdog_resets", "all_time_boxes", "history"]
                for key in required_keys:
                    if key not in data:
                        data[key] = 0 if key != "history" else []
                return data
            except (json.JSONDecodeError, ValueError) as e:
                print(f"[Stats] ไฟล์สถิติเสียหาย ({stats_path}): {e} — ใช้ค่าเริ่มต้นใหม่")
            except Exception:
                pass
        return {
            "all_time_runs": 0,
